...
ctrlShow[2001,false];
if((lbCurSel 2022) == -1) exitWith {hint "Es wurde niemand ausgewählt!";ctrlShow[2001,true];};
...
if(!life_use_atm) exitWith {hint "Du hast kürzlich die Bank ausgeraubt! Du kannst das Geld jetzt noch nicht weitergeben.";ctrlShow[2001,true];};
if(!([_amount] call TON_fnc_isnumber)) exitWith {hint "You didn't enter an actual number format.";ctrlShow[2001,true];};
if(parseNumber(_amount) <= 0) exitWith {hint "Es muss eine genaue Menge angegeben werden.";ctrlShow[2001,true];};
